...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("표준감사시간 계산기 - 2024년 12월 개정 반영"),
    ...
    h5('위험계정비중 가감요인을 적용할 때, 금융업 회사의 경우 유가증권과 파생상품, 건설업의 경우 미청구공사금액 등 회사 개별특성 및 고유환경에 따라 재고자산 및 매출채권 이외에 위험이 높은 계정이 있는 경우 이를 포함하여 위험계정비중을 고려하는 것이 더 합리적이라고 판단된다면 이들 자산을 위험계정에 포함할 수 있다.'),
    ...
    h5('자회사수를 가감요인으로 적용할 때, operation이 없어 일반적인 연결감사시간이 소요되지 않을 것으로 판단되는 종속회사, 유의적이지 않은 부문 등 중요성을 고려하여, 그 개수를 자회사수에서 적절히 차감하여 적용할 수 있다.'),
    ...
    h5('감사인은 표준감사시간을 활용하여 회사에 적용할 감사투입시간을 산정하되, 표준감사시간을 기초로 회사 개별특성 및 고유환경, 내부회계관리제도와 재무제표의 통합 감사, 학습효과, 디지털 감사 효과 등을 고려하여 최종 적용표준감사시간을 조정하여 산정할 수 있다.'),
   ...
   output$risk2 <- renderUI({
    ...
    profi <- reactive({
        ...
        weight <- if(input$bins == 0){
            ...
            1.1
        } else if(input$bins >=15 & input$bins < 20) {
            1.15
        } else if(input$bins >=20 & input$bins < 30) {
            1.2
        } else if(input$bins >=30 & input$bins < 40){
            1.1
        } else {
            1
        }
